...
func compileJetMetaprogram() {
	fmt.Printf("[GO COMPILER] Compiling Jet generator")

	handleCmdOutput(exec.Command("cd", "./tools/jet-2.12.0", "&&", "go", "build", "./cmd/jet").CombinedOutput())

	println("")
}

func generateJetModels() {
	fmt.Printf("[CODE GENERATION] Generating SQL models (jet)")

	handleCmdOutput(exec.Command("./tools/jet-2.12.0/jet", "-source=" + config.DbSource, "-dsn=" + config.DbConnectionString, "-schema=" + config.DbSchema, "-path=.jet").CombinedOutput())
