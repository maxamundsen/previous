...
func sqlCompileWhere(qb *QueryBuilder) (string, []interface{}, error) {
	var sql string
	...
	for i, v := range qb.Where {
		if !v.Unsafe && !sqlValidateName(v.Column) {
		...
		if v.Operator == BETWEEN {
			...
			if v.SubqueryBuilder != nil {
				// recurse!
				rSql, rParams, subqueryErr := sqlCompileSelect(v.SubqueryBuilder)
...
func sqlCompileSelect(qb *QueryBuilder) (string, []interface{}, error) {
	if qb == nil {
		return "", nil, errors.New("no query filter provided")
	}
	...
	// where filters
	wSql, wParams, wErr := sqlCompileWhere(qb)
	if wErr != nil {
		return "", nil, wErr
	}
...
func sqlCompileInsert(qb *QueryBuilder) (string, []interface{}, error) {
	sql := qb.BaseSQL
	...
	for i, v := range qb.Setters {
		...
		if v.SubqueryBuilder != nil {
			subSql, subParams, buildSubErr = sqlCompileSelect(v.SubqueryBuilder)
			if buildSubErr != nil {
...
func sqlCompileUpdate(qb *QueryBuilder) (string, []interface{}, error) {
	sql := qb.BaseSQL
...
func sqlCompileDelete(qb *QueryBuilder) (string, []interface{}, error) {
	sql := qb.BaseSQL

	outSql, params, err := sqlCompileWhere(qb)
	if err != nil {
		return "", nil, err
	}
...
func sqlValidateName(input string) bool {
