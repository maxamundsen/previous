...
func DocLayout(title string, displayId int, children ...Node) Node {
	return RootLayout(title+" | Previous Documentation",
		Body(Class("bg-white"), Attr("x-data", "{ mobileMenu: false }"), Attr("hx-swap", "innerHTML show:unset"),
			...
			Aside(Class("border-r-1 border-gray-200 shadow-sm bg-white fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 overflow-y-auto"),
				Div(Class("px-4 overflow-y-auto py-6"),
					A(Href(pageinfo.INDEX.Url), Img(Class("mx-auto h-16 w-auto"), Src("/images/logo.svg"), Alt("Previous"))),
			...
			Div(Class("m-5 p-10 sm:ml-72 prose prose-pre:rounded-none prose-pre:text-neutral-700 prose-pre:bg-neutral-50 prose-pre:border-1 prose-pre:border-gray-200 prose-pre:shadow max-w-none rose-a:text-neutral-700 prose-headings:text-neutral-950"),
				Group(children),
			),
		),
		Script(Raw(`
